...
class Post  {
    ...
    static async getAll (page = 1, pageSize = 10, sort = "like") {
        page = parseInt(page);
        ...
        console.log('log', pageSize, offset);

        const validSortFields = { 
            likes: '(SELECT COUNT(*) FROM likes WHERE post_id = posts.id) DESC',
            date: 'publish_date DESC'
        };
        const orderBy = validSortFields[sort] || validSortFields.likes;
    
        // Dynamically inject the ORDER BY safely
        const sql = `
            SELECT * 
            FROM posts 
            ORDER BY ${orderBy} 
            LIMIT ? OFFSET ?`;
        console.log("order sort", orderBy)
        //const sql = 'SELECT * FROM posts ORDER BY ? LIMIT ? OFFSET ?';
        console.log("sql", sql);
        try {
    ...
    static async findAll(queryOptions) {
        try {
            const { page = 1, pageSize = 10, sort = 'like' } = queryOptions;
    ...
    static async updateById (postId, title, content) {
        const sql = 'UPDATE posts SET title = ?, content = ?, status="active" WHERE id = ?';
        try {
