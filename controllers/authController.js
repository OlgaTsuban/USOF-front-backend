...
exports.sendResetLink = async (req, res) => {
    ...
    try {
        ...
        const resetLink = `http://localhost:5050/password-reset/${resetToken}`;
...
exports.uploadAvatar =  async (req, res) => {
    ...
    try {
        ...
        const avatarDbPath = `uploads/avatars/${avatarFile.filename}.png`;
        ...
        if (user.profile_picture) {
            ...
            if (fs.existsSync(previousAvatarPath) && !previousAvatarPath.includes(defaultAvatarFilename)) {
                try {
        ...
        fs.renameSync(avatarFile.path, avatarDbPath); 
...
exports.sendConfirmationEmail = async (userId, email) => {
    try {
        ...
        const confirmationLink = `http://localhost:5050/api/auth/confirm-email?token=${token}`;
