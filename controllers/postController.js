...
class PostController {
    ...
    static async getAllPosts (req, res) {
        try {
            ...
            const validSortFields = ['likes', 'date'];
            const sortColumn = validSortFields.includes(sort) ? sort : 'likes';
            const offset = (page - 1) * pageSize;
            console.log(offset);
            ...
            let posts;
            let totalPosts;
            console.log("sorting col", sortColumn);
            if (isAdmin == 'admin') {
                // Admin can see all posts
                posts = await Post.getAll(page, pageSize, sortColumn);
                ...
                posts = await Post.findAll(
                    //userId,
                    { limit: pageSize, offset: offset, sort:sortColumn, where: {
                        author_id: userId, // Filter by author ID
                    }, }
                );
    ...
    static async updatePost (req, res) {
        ...
        try {
            ...
            if (userRole == 'admin' && post.author_id !== authorId) {
    ...
    static async deletePost (req, res) {
        ...
        try {
            ...
            if (post.author_id !== userId && req.session.user.role == 'user') {
    ...
    static async lockPostById (req, res) {
        ...
        console.log('Comment ID:', postID);
            ...
            if (typeof lock === 'undefined') {
                return res.status(400).json({ message: 'Lock status is required to update the post.' });
            }
        /*if (!lock ) {
            return res.status(400).json({ message: 'Content/status is required to update the post.' });
        }*/
